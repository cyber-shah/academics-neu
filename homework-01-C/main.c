...
int main(int argc, char const *argv[])
{
    ...
    printf("\nHello %s", name); // This is a function call, it calls the printf function, which prints the name variable to the command line

    if(number >= 21) { // C uses the basic conditions of >=, >, <=, <, ==, != (not equal to). For logical operators it uses && (and), || (or), ! (not)
        printf("\nYou are old enough to drink in the US"); // This is a function call, it calls the printf function, it needs double quoted strings
    } else {
        printf("\nYou are not old enough to drink in the US"); // This is a function call, it calls the printf function
    }
